...
def run(params: Dict[str, Any], outputs_dir: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return _error(out_dir, reason_key="output_write_failed", message=str(e))

    return {"files": ["results.jsonl", "report.json"], "metadata": {"total_written": total_written}}


def _error(out_dir: Path, reason_key: str, message: str) -> Dict[str, Any]:
    # Best-effort write of a failure report without raising.
    try:
        (out_dir / "report.json").write_text(
            json.dumps({"module_id": MODULE_ID, "status": "failed", "reason_key": reason_key, "message": message}, ensure_ascii=False, indent=2)
            + "\n",
            encoding="utf-8",
        )
    except Exception:
        pass
    return {"files": ["report.json"], "status": "failed", "reason_key": reason_key, "message": message}
