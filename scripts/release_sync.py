"""Sync GitHub Releases into repo's /releases directory with obfuscated folder names.

Key requirements covered:
  - Billing release tag stays FIXED: billing-state-v1
  - All other releases are materialized into /releases/<release_alias_id>/
  - Maintain a mapping table:
        .platform/internal/release_id_map.csv
    mapping 8-char Base62 alias -> GitHub numeric release id

Notes:
  - Requires `gh` CLI available and authenticated via GH_TOKEN.
  - Works on GitHub-hosted runners (gh is preinstalled) and can be used on self-hosted runners
    if gh is installed.
"""
...
from .id_manager import IdManager, ReleaseIdMap

BILLING_STATE_TAG = "billing-state-v1"
