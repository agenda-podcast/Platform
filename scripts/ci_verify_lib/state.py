...
def _validate_maintenance_state(repo_root: Path) -> None:
    ...
    for rel in required:
        p = ms / rel
        if not p.exists():
            core._fail(f"Missing maintenance-state file: {rel}")
    ...
    for r in cat:
        ...
        if scope not in ("GLOBAL", "MODULE"):
            core._fail(f"Invalid reason scope: {scope!r}")

        mid = str(r.get("module_id", "")).strip()
        if scope == "MODULE":
            validate_id("module_id", mid, "module_id")
        elif mid:
            core._fail("Global reason has non-empty module_id")

    # Header spot-check: modules index (used for dropdowns)
    core._assert_exact_header(
        ms / "modules_index.csv",
        ["module_id", "name", "kind", "version", "supports_downloadable_artifacts", "path"],
    )

    core._ok("Maintenance-state: required files + ID format OK")
...
def _validate_billing_state(billing_state_dir: Path) -> None:
    ...
    for fn in required_files:
        ...
        if not p.exists():
            core._fail(f"Billing-state missing required file: {p}")

    # headers
    core._assert_exact_header(
        billing_state_dir / "tenants_credits.csv",
        ["tenant_id", "credits_available", "updated_at", "status"],
    )
    core._assert_exact_header(
        billing_state_dir / "transactions.csv",
        [
            "transaction_id",
            "tenant_id",
            "work_order_id",
    ...
    core._assert_exact_header(
        billing_state_dir / "transaction_items.csv",
        [
            "transaction_item_id",
            "transaction_id",
            "tenant_id",
    ...
    core._assert_exact_header(
        billing_state_dir / "promotion_redemptions.csv",
        ["redemption_id", "tenant_id", "promo_code", "credits_granted", "created_at", "note", "metadata_json"],
    )
    core._assert_exact_header(
        billing_state_dir / "cache_index.csv",
        ["place", "type", "ref", "created_at", "expires_at"],
    )
    core._assert_exact_header(
        billing_state_dir / "github_releases_map.csv",
        ["release_id", "github_release_id", "tag", "tenant_id", "work_order_id", "created_at"],
    )
    core._assert_exact_header(
        billing_state_dir / "github_assets_map.csv",
        ["asset_id", "github_asset_id", "release_id", "asset_name", "created_at"],
    )
    ...
    core._ok("Billing-state: required assets + headers + basic ID format OK")
